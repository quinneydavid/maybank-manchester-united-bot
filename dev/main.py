...
import requests
import json
from telegram import Bot  # Add this line
...
def setup_logging():
    ...
    # File handler
    file_handler = logging.FileHandler('/app/oddsapi.log')
...
def load_cache():
    ...
    if os.path.exists(CACHE_FILE) and os.path.getsize(CACHE_FILE) > 0:
        try:
            ...
            logger.error(f"Error reading {CACHE_FILE}. Using empty cache.")
            cache = {}
    else:
        logger.info(f"{CACHE_FILE} not found or empty. Using empty cache.")
        cache = {}
...
def get_upcoming_matches():
    ...
    if response.status_code != 200:
        logger.error(f"Failed to get data: status_code {response.status_code}, response body {response.text}")
        return None
...
# Fix: Make check_upcoming_matches async and use await
...
# Fix: Make analyze_match async and use await
...
{"Consider using your card for purchases today for potential bonus points and cashback!" if win_probability > 50 else "The win probability is low. Use your card with caution today."}
    """
...
async def check_match_result():
    ...
    if matches:
        ...
        if (current_time_utc - match_end_time_utc) <= timedelta(hours=24):
            ...
            logger.info("Most recent match ended more than 24 hours ago. No notification sent.")
    else:
        logger.info("No recent Manchester United match result found.")
